...
const VideoCard = (props: { id: number; title: string; thumbnail_url: string; user_name: string; profile_url: string; upload_date: string }) => {
    return (
        <Flex bg="gray.600" p={5} w="unset" alignItems="center" justifyContent="center">
            <Box mx="auto" rounded="lg" shadow="md" bg={useColorModeValue('white', 'gray.800')} maxW="2xl">
                ...
                <Box p={6}>
                    <Box>
                        <Link display="block" color={useColorModeValue('gray.800', 'white')} fontWeight="bold" fontSize="2xl" mt={2} _hover={{ color: 'gray.600', textDecor: 'underline' }} href={"#/watch/" + props.id}>
                            {props.title}
                        </Link>
                    </Box>
